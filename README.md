...
- **数据库规范**：MSSQL/Oracle/MySQL/PostgreSQL 四库建表与查询（08-11、14-17）；另有 12 号 PDA 前端（Vue2）精炼规则
- **18-23 配方库**：蒸馏自 SMOM 开发手册 2026-08 版（Web 命令 22 篇 / Web 行为与编辑器 13 篇 / Web 前端 12 节 / WPF 6 组 / 服务端工具 12 节 / 问题库 11 节）
...
| 领域 | 内容 |
|------|------|
| 架构 | 分层架构、Module 注册、DataProvider、IoC（RT.Service/RF/DB）、类命名规范、事务规则 |
...
| Web 命令配方 | 保存/提交/导入导出/打印/添加/选择/弹窗查看全配方（22 篇，按任务域拆为 `18-web-commands-form` / `import-export` / `add-lookup` / `print-attach` 4 份） |
| Web 行为与编辑器 | 单元格/行变色、状态栏、分页、排序、统计行、模糊枚举/枚举多选编辑器、透视表动态列（13 篇，`19-web-behaviors.md`） |
| Web 前端进阶 | invokeCommand、mon/fireEvent/mun、SIE.Msg、三档客制化（LayoutClass/UIGenerator/ModuleRuntime）、DM_ 跨层传参（12 节，`20-web-frontend-misc.md`） |
| WPF 端 | ViewConfig、ViewBehavior、ListViewCommand、PagingLookUpEditor、Layout；弹窗命令/行为/编辑器/控件/工具（6 组，`21-wpf-recipes.md`） |
| 数据库 | MSSQL 建表/查询、Oracle 建表/查询、MySQL 建表/查询、PostgreSQL 建表/查询、类型映射、序列、索引 |
| 服务端工具 | ObjectUtil/EntityUtil/DataChecker/SqlCreator/DbBulkProvider、调度防并发、预警三件套、钉钉/邮件/企微推送、客制化 WebApi（12 节，`22-host-tools-cases.md`） |
| 工作流与问题排查 | Elsa 工作流全流程（节点/转办驳回/通知）、查询效率、框架开关、配置缓存、BS 弹窗子页签（11 节，`23-problems.md`） |
