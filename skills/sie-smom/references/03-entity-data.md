...
> **列名自动推导红线**：`MapAllProperties()` 下列名由属性名驼峰转全大写下划线，**连续大写字母逐个拆分**——`RelatedSN` 推导为 `RELATED_S_N`、`SourceSN` 为 `SOURCE_S_N`（不是直觉的 `*_SN`）。建表 DDL 列名必须与推导结果一致，否则实体查询运行时报 `ORA-00904: 标识符无效`（属性注册不报编译错，纯运行时爆）。列名无法按推导名建时，用 `Meta.Property(X).MapColumn("实际列名")` 显式映射兜底。
