...
- **数据底座**：所有业务实体继承 `DataEntity`，逻辑删除用 `IS_PHANTOM`（bool→0/1），主键 ID 与外键均为 `FLOAT`(MSSQL) / `NUMBER(18,0)`(Oracle) / `BIGINT`(MySQL/PostgreSQL)，由**序列**生成（不用 IDENTITY）；MySQL 用 `AUTO_INCREMENT`、PostgreSQL 用 `IDENTITY`。
...
| WPF（ViewConfig / Behavior / Command / Editor / Layout） | `02-wpf.md` + `21-wpf-recipes.md`（弹窗命令/行为/编辑器/控件/工具配方 28 篇） |
...
|---|---|---|---|---|---|---|
| ID | long | FLOAT NOT NULL | NUMBER(18,0) NOT NULL | BIGINT NOT NULL AUTO_INCREMENT | BIGINT IDENTITY | 主键 |
| SYNC_ID | long | FLOAT NOT NULL | NUMBER(18,0) NOT NULL | BIGINT NOT NULL | BIGINT NOT NULL | 同步ID |
| CREATE_BY | long? | FLOAT NULL | NUMBER(18,0) | BIGINT NULL | BIGINT NULL | 创建人 |
| CREATE_DATE | DateTime | DATETIME NOT NULL | DATE NOT NULL | DATETIME NOT NULL | TIMESTAMP NOT NULL | 创建时间 |
| UPDATE_BY | long? | FLOAT NULL | NUMBER(18,0) | BIGINT NULL | BIGINT NULL | 更新人 |
...
|---|---|---|---|---|
| long / IRefIdProperty | FLOAT | NUMBER(18,0) | BIGINT | BIGINT |
...
├── 22-host-tools-cases.md     # 服务端工具方法与经验案例（DataChecker/CommonEntityController/SqlCreator/DbBulkProvider 等，20 篇）
└── 23-problems.md             # 常见问题与解决方案（工作流实战/框架开关/配置缓存/排查清单，14 篇）
```
